...
\author{Sicheng Ma \\ \texttt{sm3035}}
\date{\today}
...
%------------------------------------------------------------------------------
\noindent\textbf{Team:} chmawa. \\
\noindent\textbf{Members:} Sicheng Ma, Baiyuan Chen, Zixuan Wang.\\
\textbf{Jointly-owned experiments (I.1--I.3):} 
% \textit{[state which runs / plots are shared with teammates; everything else, and all of I.4 and Part~II, is your own individual work]}.
\\
\textbf{Code (GitLab):} \url{https://gitlab.com/USER/REPO}\quad % TODO: replace with your GitLab repo
\textbf{W\&B:} \href{https://wandb.ai/sichengma0514-university-of-cambridge/a8-grpo}{\texttt{a8-grpo}} (baseline run \href{https://wandb.ai/sichengma0514-university-of-cambridge/a8-grpo/runs/bdbugenj}{\texttt{bdbugenj}})\\
...
$32$\,GiB HBM). The run completed all \texttt{MAX\_STEPS}$=3364$ GRPO steps
($=\lfloor 0.9\times\texttt{NUM\_BATCHES}\rfloor$, the $90\%$ train split of
$3738$ micro-batches; with group size $K=2$ that is $6728$ rollouts) in a
wall-clock time of $\approx\!4.7$\,h ($16{,}999$\,s end-to-end; $\approx\!4.6$\,h
inside the training loop), logged to W\&B run
\href{https://wandb.ai/sichengma0514-university-of-cambridge/a8-grpo/runs/bdbugenj}{\texttt{bdbugenj}}.
Salient defaults: $\beta=0.08$, $\varepsilon=0.2$, LoRA rank${=}\alpha{=}64$,
learning rate $3\times10^{-6}$ (cosine, $10\%$ warmup), grad-norm clip $0.1$,
micro-batch $1$, rollout temperature $0.9$, up to $768$ generated tokens.
...
(\texttt{NUM\_TEST\_BATCHES}), scored greedily
(\texttt{evaluate.py --preset greedy}: \texttt{temperature}{=}\texttt{None},
\texttt{top\_k}{=}1), so every number is deterministic. ``Exact'' compares the
parsed \texttt{<answer>} to the gold number, ``$\pm10\%$'' uses the numeric ratio,
and ``Format'' is the fraction of completions matching the
\texttt{<reasoning>}\dots\texttt{</reasoning><answer>}\dots\texttt{</answer>}
template. Because only post-peak checkpoints survived
(\texttt{SAVE\_INTERVAL\_STEPS}{=}500, \texttt{MAX\_TO\_KEEP}{=}4), we report the
final checkpoint (step $3364$, ``the resulting checkpoint'') alongside steps
$3000$ and $2000$ to show the trend.
\begin{table}[h]
  \centering
  \caption{GSM8K accuracy: base \texttt{gemma-3-1b-it} vs.\ the baseline LoRA
  checkpoints (GSM8K test, first $N{=}64$ problems, shuffle \texttt{seed}{=}42,
  greedy). Exact-match counts of $64$: base $33$, step~$2000$ $18$, step~$3000$
  $4$, step~$3364$ $2$.}
  ...
  \begin{tabular}{lccc}
    ...
    Baseline LoRA, step $3000$               & 6.25  & 7.81  & 12.50 \\
...
model already solves $51.6\%$ of the problems --- it obeys the \texttt{<answer>}
instruction well enough to be parsed, but rarely emits the full strict template,
hence its low \emph{format} score --- whereas every saved checkpoint is worse and
degrades monotonically, the ``resulting'' step-$3364$ checkpoint collapsing to
$3.1\%$. Tellingly, step $2000$ has the \emph{highest} format accuracy ($35.9\%$)
yet still lower correctness than base: the format-shaping reward is being
optimised at the expense of the arithmetic (reward hacking) before the run
destabilises entirely (Fig.~\ref{fig:baseline-curves}).

\paragraph{Training curves.}
The reward (Fig.~\ref{fig:baseline-curves}, left; $\bar r$ summed over the four
reward functions, max $10$) rises fast --- the policy acquires the output
template within $\sim$150 steps and peaks at $\bar r\approx 6$--$7$ near step
$450$ --- but then \emph{declines steadily} for the rest of training, reaching
$\bar r\approx 0$ (often negative) by step $\sim$2700 with no recovery. The
held-out (val) reward, logged every $64$ steps, tracks training closely, so this
is genuine policy degradation, not over-fitting. KL stays small ($\lesssim 0.7$)
for most of the run but shows sharp spikes (up to $\approx 41$, inset) in the
final third --- the signature of unstable updates. Every surviving checkpoint
therefore lies in this post-peak regime, which the accuracies in
Table~\ref{tab:baseline-acc} reflect; this instability is what the I.3
modification targets.
\begin{figure}[h]
  \centering
  \includegraphics[width=0.49\textwidth]{baseline_reward.pdf}\hfill
  \includegraphics[width=0.49\textwidth]{baseline_kl.pdf}
  \caption{Baseline run \texttt{bdbugenj}: mean rollout reward $\bar r$ (left; max
  $10$) and $\mathrm{KL}(\pi_\theta\,\|\,\pi_{\mathrm{ref}})$ (right, with a
  full-range inset for the late spikes) vs.\ GRPO step. Faint = per-step; bold =
  $100$-step running mean. Reward peaks near step $450$ then collapses.}
  \label{fig:baseline-curves}
\end{figure}

\paragraph{Baseline patches.}
Training ran as-shipped: at launch the tree matched \texttt{324abbe} except a
two-line home-directory path edit in the \texttt{run\_tmux.sh} launcher;
the GRPO algorithm and \texttt{config.py} were \emph{unchanged}. Three minor
fixes, all outside the training loop: \textbf{(1) W\&B routing} ---
\texttt{config.py} defaults to another account, so we set
\texttt{WANDB\_ENTITY}/\texttt{WANDB\_PROJECT} via environment (run logged under
\texttt{sichengma0514-university-of-cambridge/a8-grpo}); \textbf{(2) evaluation
restore} --- as shipped, \texttt{evaluate.py} wraps the base model in a fresh
LoRA adapter but never loads a checkpoint, and a zero-initialised adapter is a
no-op ($\Delta W{=}\tfrac{\alpha}{r}BA$ with $B{=}0$), so it only ever scored the
\emph{base} model; we added \texttt{-{}-ckpt-dir/-{}-steps/-{}-include-base} flags
that restore the adapter through Tunix's \texttt{CheckpointManager} (mirroring the
existing \texttt{chat.py}), leaving the default invocation unchanged;
\textbf{(3) dataset re-read} --- re-opening the cached TFDS dataset in a fresh
process throws \texttt{AttributeError: 'FieldDescriptor' object has no attribute
'label'} under the installed \texttt{protobuf}~7.34 (the attribute was removed in
\texttt{protobuf}~$\ge$5), so before evaluating we delete the stale prepared
\texttt{data/\{train,test\}/gsm8k/1.0.0} and let \texttt{evaluate.py} re-prepare
from the cached raw download in-process --- the same code path training used ---
needing \emph{no code change}. (We also copied the volatile
\texttt{/tmp/content/ckpts} to \texttt{\textasciitilde/tpu-2026/ckpts\_backup}.)
...
The group size is $K=2$: \texttt{NUM\_GENERATIONS} in \texttt{config.py}, passed as
\texttt{GRPOConfig(num\_generations$=$2)} in \texttt{train.py}, so each prompt is rolled out
twice and the pair forms one group. Tunix resolves the estimator by name from a
registry: \texttt{GRPOConfig.advantage\_estimator} defaults to \texttt{"grpo"} and
\texttt{train.py} does not override it, which selects \texttt{compute\_advantages} in
\texttt{tunix/rl/algo\_core.py} --- the \emph{standard} group baseline
$\hat A_i=(r_i-\bar r)/(\sigma_r+10^{-6})$ with the group's sample std
(\texttt{ddof}$=$1), \emph{not} leave-one-out (RLOO ships in the same file under the
separate registry name \texttt{"rloo"}). A consequence of $K{=}2$: $r_i-\bar r=
\pm\tfrac12(r_1-r_2)$ and $\sigma_r=|r_1-r_2|/\sqrt2$, so whenever the two rewards differ
at all, $\hat A_i=\pm1/\sqrt2$ \emph{regardless of the margin}, and $\hat A_i\approx0$ on
ties --- the advantage carries only the sign of the within-pair comparison.
...
\emph{fallback} that also shapes towards emitting an extractable number at all. With the
correctness reward alone, early in training the per-rollout success probability $p$ is tiny
(doubly so for \texttt{check\_answer}, which is gated on the template parsing), so both
...
$|\hat A_i|=1/\sqrt2$) from the first steps; this is what lets training start, and equally
what the run later exploits when format is optimised at the expense of correctness
(Sec.~\ref{sec:reproduce}).
...
The clip is implemented in \texttt{grpo\_loss\_fn} in \texttt{tunix/rl/algo\_core.py}
(registered as policy loss \texttt{"grpo"}): per-token log-ratios are exponentiated to
$\rho_t$ and the per-token loss is
$\max\bigl(-\hat A\,\rho_t,\,-\hat A\,\mathrm{clip}(\rho_t,1-\varepsilon,1+\varepsilon)\bigr)$,
the lecture objective with $\varepsilon=\texttt{EPSILON}=0.2$ from \texttt{config.py} (via
\texttt{GRPOConfig(epsilon$=$0.2)}; the upper bound $\varepsilon_{\mathrm{high}}$ defaults to
$\varepsilon$, so the clip is symmetric). Two deviations from the lecture form. \emph{(1)
...
with $\mu=\texttt{NUM\_ITERATIONS}=1$ Tunix never computes separate $\pi_{\mathrm{old}}$
log-probs (\texttt{grpo\_learner.py} does so only for $\mu>1$) and substitutes
\texttt{stop\_gradient} of the current log-probs, so $\rho_t\equiv1$ in the forward pass, the
clip never binds (\texttt{pg\_clipfrac}$=0$), and the surrogate reduces to group-relative
REINFORCE, $-\hat A_i\nabla_\theta\log\pi_\theta$. The KL anchor is a separate additive
penalty $\beta\,\widehat{\mathrm{KL}}(\pi_\theta\|\pi_{\mathrm{ref}})$ with $\beta=0.08$,
estimated per token by the naive $\log\pi_\theta-\log\pi_{\mathrm{ref}}$ (mode
\texttt{"kl"}: unbiased but sign-indefinite --- and indeed ${\sim}1.5\%$ of the logged
steps dip marginally below zero).
